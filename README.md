...
- **界面体验优化 (UI/UX)**:
    ...
    - **2026-03-11 dev-5a660a**
      - 修复了全平台按下 Tab 键导致界面向上偏移错位的问题（实现了手动焦点管理与禁止原生滚动）
      - 修复了表情块和外部框架没有对齐的问题
      - [Android]自动重连 
    - **2026-03-12**
      - [tauri]修复了鼠标放到消息列表上hover效果没有生效的问题
      - Android 端在缩放模式下键盘弹出导致的布局缩进问题
      - [tauri]修复了上部程序框遮住了导航栏上部和消息/联系人列表上部的问题
      - 增加了小黄脸CDN和手动下载的选项 精简安装包体积！
      - 修复@用户栏无法点击空白区域关闭
      - [Android]修复点击消息框输入弹出键盘后 拉住消息框上下移动可以把整个页面都滑动起来
      - 删除触摸端右滑回复 它总是和右滑返回互相折磨
      - [Android]启用了 WebView 远程调试
      - 交互与手势: 移除了手势冲突，优化了外链访问逻辑与复制体验。
      - 卡片消息: 简化了卡片渲染，修复了点击失效问题，并统一了样式。
      - [Android]修复了@用户的时候，出现的用户栏圆角太大
    - **2026-03-13**
      - [Web/Desktop]修复平板模式、移动端模式UI逻辑
      - 更深邃的黑暗模式！
    - **2026-03-14**
      - 全平台CSS重构完成！太累了！简直是屎山！
      - 修改添加和放出群收纳盒的逻辑
    ...
    - **2026-03-16**
      - 修复最新一张图片预览错误的bug 
      - 修复图片+1复读后无法在消息框加载的bug
      - 文件上传成功后自动刷新聊天
      - 截图隐私模式 一键屏蔽所有消息内容以外的头像和文字
      - 右键群聊用户预览用户头像
      - 修复群聊用户悬浮窗没有像右键菜单那样和缩放定位准确位置的bug
      - 修复双击复制没有覆盖到合并转发内的消息
      - 修复复制文字窗口不接受安卓返回键
      - 微调CSS
      - 通知弹窗无法用安卓返回键返回
      - 修复私聊时自己撤回消息错误显示为对方撤回的问题
      - 表情回应：现在可以点击消息下方的 emoji 气泡来添加或取消回应，已回应的气泡会高亮显示
      - 表情回应通知：收到他人的表情回应时，通知消息中会附带具体的 emoji 图标
      - 回应通知显示：聊天中可查看来自其他用户的表情回应通知记录
      - 组件标签：修复多个组件中 HTML 标签未正确闭合的问题
