...
<script setup lang="ts">
    ...
    const colorHex = computed(() => hsvToHex(hue.value, saturation.value, value.value))
    const historyColors = computed(() => props.historyColors.map((item) => normalizeHexColor(item)))

    watch(() => props.modelValue, (nextColor) => {
